...
if ( ! class_exists( 'WC_Woo_Import_Product' ) ) {
	class WC_Woo_Import_Product {
    ...
    private $_file_csv_remote;  	
		...
		public function __construct(){
            ...
            global $wpdb;
  			...
  			// richiamo i settaggi dalle opzioni
  			$woo_import_products_main 	  = get_option( 'woo_import_products_main' );
  			$this->_file_csv_remote 			= $woo_import_products_main["file_csv_remote"];
  			$this->_admin_mail 				    = $woo_import_products_main["admin_mail"];
  			$this->_from_mail 				    = $woo_import_products_main["from_mail"];
			
			  // inizializzo le due tabelle
            $this->_tbl_importa_db 		= $wpdb->prefix . "importa_db";
            $this->_tbl_conversione 	= $wpdb->prefix . "conversione";
			
			// abilito la directory del plugin
